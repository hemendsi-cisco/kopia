...
import (
	...
	"github.com/kopia/kopia/repo/blob/awss3"
)

type storageAWSS3Flags struct {
	s3options awss3.Options
}

func (c *storageAWSS3Flags) Setup(svc StorageProviderServices, cmd *kingpin.CmdClause) {
	...
	cmd.Flag("use-accelerate", "Use S3 Transfer Acceleration").BoolVar(&c.s3options.UseAccelerate)
...
func (c *storageAWSS3Flags) Connect(ctx context.Context, isCreate bool, formatVersion int) (blob.Storage, error) {
	_ = formatVersion
	...
	//nolint:wrapcheck
	return awss3.New(ctx, &c.s3options, isCreate)
}
